...
async def link_with_channel_check(request: Request):
    ...
    referral_code = data.get("referral_code", "")  # ADD THIS
    ...
    # FIXED: Check if this telegram_id is already linked to ANOTHER email
    ...
    if existing_telegram.data:
        for record in existing_telegram.data:
            if record.get("email") != email:
                # This Telegram is linked to a different email
                other_email = record.get("email", "unknown")
                raise HTTPException(
                    status_code=400, 
                    detail=f"This Telegram account is already linked to another email ({other_email[:3]}***). Each Telegram account can only be linked to one badge."
                )
    
    # Check if the email user exists
    email_user = supabase.table("badge_users").select("*").eq("email", email).execute()
    if not email_user.data:
        raise HTTPException(status_code=404, detail="Please verify your email first")
