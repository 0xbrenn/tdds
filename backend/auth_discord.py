...
async def discord_callback(request: Request):
    ...
    try:
        ...
        if existing_discord.data:
            ...
            for record in existing_discord.data:
                if email and record.get("email") != email:  # Use email instead of state
    ...
    try:
        if email:  # Use email instead of state
            ...
            if existing_discord_users.data:
                for record in existing_discord_users.data:
                    if record.get("email") != email:  # Use email instead of state
                        # Don't allow overwriting
                        other_email = record.get("email", "unknown")
                        error_msg = f"This Discord account is already linked to {other_email[:3]}***. Use a different Discord account."
                        encoded_msg = urllib.parse.quote(error_msg)
                        redirect_url = f"{FRONTEND_URL}?platform=discord&status=duplicate&message={encoded_msg}"
                        if referral_code:
                            redirect_url += f"&ref={referral_code}"
                        return RedirectResponse(url=redirect_url)
            
            # Check if email user exists
            existing = supabase.table("badge_users").select("*").eq("email", email).execute()  # Use email instead of state
            ...
            if existing.data:
                ...
                }).eq("email", email).execute()  # Use email instead of state
