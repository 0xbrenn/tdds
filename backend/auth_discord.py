...
async def discord_callback(request: Request):
    ...
    try:
        if state:  # If email was provided
            ...
            if existing.data:
                ...
                print(f"✅ Created new user with email: {state}")
        else:
    ...
    if is_member:
        return RedirectResponse(
            url=f"{FRONTEND_URL}?platform=discord&status=success&username={urllib.parse.quote(global_name or username)}"
        )
    else:
        # Make sure to URL encode the invite link
        encoded_invite = urllib.parse.quote(DISCORD_INVITE_LINK)
        return RedirectResponse(
            url=f"{FRONTEND_URL}?platform=discord&status=not_member&invite={encoded_invite}&username={urllib.parse.quote(global_name or username)}"
        )

@router.get("/status/{discord_id}")
async def check_discord_status(discord_id: str):
