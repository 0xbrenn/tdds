...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_channel_member:
        ...
        if email:
            ...
            response += f"🌐 Return to the website to continue: {FRONTEND_URL}"
        else:
    ...
    if email and FRONTEND_URL and not FRONTEND_URL.startswith("http://localhost"):
        keyboard.append([InlineKeyboardButton("🌐 Back to Website", url=FRONTEND_URL)])
    
    reply_markup = InlineKeyboardMarkup(keyboard) if keyboard else None
    
    # If using localhost, add the URL to the message text instead
    if email and FRONTEND_URL.startswith("http://localhost"):
        response += f"\n\n🔗 Continue at: {FRONTEND_URL}"
                    ...
                    if email:
                        ...
                        if update_response.ok:
                            ...
                            if FRONTEND_URL.startswith("http://localhost"):
                                success_message += f"🔗 Continue at: {FRONTEND_URL}"
