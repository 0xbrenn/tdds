...
async def register_instant(request: EmailRequestWithReferral):
    ...
    try:
        ...
        new_user = {
            "email": request.email,
            "email_added": True,
            "email_verified_at": datetime.now().isoformat(),  # Mark as verified instantly
            "badge_issued": False,
            "telegram_joined": False,
            "discord_joined": False,
            "twitter_followed": False,
        ...
        if result.data:
            print(f"✅ Instant registration successful for {request.email}")
            return {
                "success": True,
        ...
        print(f"Error in instant registration: {str(e)}")
        # Check if it's a duplicate key error
        if "duplicate key" in str(e):
