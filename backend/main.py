...
# Create cache instances (only used if Redis not available)
if not REDIS_AVAILABLE:
    ...
    dashboard_cache = SimpleCache(ttl_seconds=60)
...
async def check_user_status(request: Request, email: str):
    ...
    try:
        ...
        if not result.data:  # <-- THIS LINE WAS NOT INDENTED CORRECTLY
            response = {
                "exists": False,
                "email": email,
                "tasks": {
            ...
            # Cache non-existent users for shorter time (60 seconds)
            if REDIS_AVAILABLE and cache:
                await cache.set_async(f"status:{email}", response, ttl=60)
            else:
        ...
        # Cache the result
        if REDIS_AVAILABLE and cache:
            await cache.set_async(f"status:{email}", response, ttl=300)
        else:
            status_cache.set(f"status:{email}", response)
