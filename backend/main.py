...
async def claim_badge_with_referral(request: Request):
    ...
    try:
        ...
        # Issue badge
        update_result = supabase.table("badge_users").update(update_data).eq("email", email).execute()
        
        # Process referral reward if applicable
        referral_reward = None
        if referral_code and not user.get("referred_by"):  # Only process if this is a new referral
            # Find the referrer
            referrer = supabase.table("badge_users").select("*").eq("referral_code", referral_code).execute()
            ...
            if referrer.data:
                ...
                if drop:
                    ...
                    # Note: successful_referrals is updated automatically by database trigger
