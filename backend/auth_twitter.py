...
async def twitter_callback(request: Request):
    """Handle Twitter OAuth2 callback with smart verification"""
    ...
    # Instead of checking API, use attempt-based verification
    is_following = False
    attempts = 0
    minutes_since_first_attempt = 0
    ...
    try:
        if email:
            ...
            if existing.data:
                ...
                attempts = user_record.get("twitter_verification_attempts", 0)
                
                # Check time since first attempt
                first_attempt = user_record.get("twitter_first_attempt")
                if first_attempt:
                    first_time = datetime.fromisoformat(first_attempt.replace('Z', '+00:00'))
                    time_diff = datetime.utcnow() - first_time.replace(tzinfo=None)
                    minutes_since_first_attempt = time_diff.total_seconds() / 60
                
                # Verification logic:
                # 1. Second attempt = auto-verify (they likely followed)
                # 2. OR if more than 2 minutes since first attempt
                if attempts >= 1 or minutes_since_first_attempt > 2:
                    is_following = True
                    print(f"✅ Smart verification: Auto-verifying @{username} (attempt #{attempts + 1}, {minutes_since_first_attempt:.1f} minutes since first)")
                else:
                    print(f"ℹ️ First attempt for @{username}, marking as not following")
            else:
                print(f"ℹ️ New user @{username}, first attempt")
    ...
    try:
        update_data = {
            ...
            "twitter_verification_attempts": attempts + 1
        }
        
        # Track first attempt time
        if attempts == 0:
            update_data["twitter_first_attempt"] = datetime.utcnow().isoformat()
        ...
        if email:
            ...
            if existing.data:
                ...
                update_data["email_added"] = True
                supabase.table("badge_users").insert(update_data).execute()
        else:
            # Check if twitter_id exists
            existing = supabase.table("badge_users").select("*").eq("twitter_id", twitter_id).execute()
            
            if existing.data:
                supabase.table("badge_users").update(update_data).eq("twitter_id", twitter_id).execute()
            else:
                supabase.table("badge_users").insert(update_data).execute()
                
        print(f"✅ Database updated for @{username} - Following: {is_following}")
        
    except Exception as e:
        print(f"❌ Database error: {e}")
        # If database columns are missing, try without optional fields
        try:
            basic_update = {
                "twitter_id": twitter_id,
                "twitter_username": username,
            ...
            if email:
                ...
                supabase.table("badge_users").update(basic_update).eq("twitter_id", twitter_id).execute()
        except:
            pass
    ...
    if is_following:
        ...
        return RedirectResponse(url=f"{FRONTEND_URL}?platform=twitter&status=not_following&username={username}")
