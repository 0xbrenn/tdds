...
async def twitter_login(request: Request):
    ...
    # Store state with email and verifier
    auth_states[state] = {
        "email": email,
    ...
    try:
        if email:
            ...
            if existing_twitter_users.data:
                for record in existing_twitter_users.data:
                    if record.get("email") != email:
                        ...
                        encoded_msg = urllib.parse.quote(error_msg)
                        return RedirectResponse(
                            url=f"{FRONTEND_URL}?platform=twitter&status=duplicate&message={encoded_msg}"
                        )
            ...
            if existing.data:
                ...
                return RedirectResponse(url=f"{FRONTEND_URL}?platform=twitter&status=error&message=user_not_found")
        else:
