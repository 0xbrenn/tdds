...
useEffect(() => {
  ...
  if (platform && status) {
    ...
    } else if (status === 'not_member' && platform === 'discord') {
      // Discord specific - not a member
      setDiscordAttempted(true);
      setDiscordInviteLink(invite || '');
      
      const decodedMessage = message ? decodeURIComponent(message) : 'You need to join the IOPn Discord server first.';
    ...
    if (response.ok && data.success) {
      ...
      if (data.referral_reward) {
        ...
        showNotification(
          ...
          `They received a ${drop.tier.charAt(0).toUpperCase() + drop.tier.slice(1)} tier drop for referring you.`
        );
      }
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900/95 border border-purple-500/30 rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
            <div className="mb-4">
              <div className="w-16 h-16 bg-purple-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <Clock className="w-8 h-8 text-purple-400" />
              </div>
              <h2 className="text-2xl font-bold mb-2">We'll Be Back Soon!</h2>
              <p className="text-gray-300 mb-4">
                The Early n-Badge claiming process is temporarily paused for maintenance.
              </p>
              <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4 mb-4">
                <p className="text-lg font-semibold text-purple-300">
                  Resuming Monday, July 21
                </p>
        ...
        <div className="relative z-10 max-w-2xl w-full text-center">
          ...
          <button
            disabled={true}
            className="px-6 py-3 bg-gray-600 text-gray-400 rounded-xl font-semibold cursor-not-allowed opacity-50 inline-flex items-center gap-2"
          >
            Get Started
          ...
          <p className="mt-4 text-sm text-gray-400">
            ...
            <button
              disabled={true}
              className="ml-2 text-gray-500 cursor-not-allowed opacity-50 underline"
            >
              Login here
            </button>
          </p>
